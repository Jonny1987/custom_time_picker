...
// Forked from Flutter's Material time picker implementation:
// /home/john/fvm/versions/3.29.3/packages/flutter/lib/src/material/time_picker.dart
//
// This file is vendored so we can add min/max time range restrictions while
// preserving the default Material time picker UX.
...
Color _withOpacity(Color color, double opacity) {
  // Prefer Color.withValues() over the deprecated withOpacity().
  return color.withValues(alpha: opacity.clamp(0.0, 1.0));
}

TextStyle? _disabledTextStyle(TextStyle? style) {
  if (style == null) {
    return null;
  }
  final Color base = style.color ?? const Color(0xFF000000);
  return style.copyWith(color: _withOpacity(base, 0.38));
}

/// Allowed minute selection increments for the picker.
enum MinuteInterval {
  five(5),
  ten(10),
  fifteen(15),
  ...
  const MinuteInterval(this.minutes);
  final int minutes;
}

int _timeOfDayToMinutes(TimeOfDay time) => time.hour * 60 + time.minute;

bool _isMinuteSelectable(int minute, MinuteInterval? minuteInterval) {
  final int? step = minuteInterval?.minutes;
  if (step == null) {
    return true;
  }
...
int _roundMinuteToInterval(int minute, int interval) {
  // Round to the nearest interval (ties round up).
  final int half = interval ~/ 2;
  return ((minute + half) ~/ interval) * interval % TimeOfDay.minutesPerHour;
}
...
bool _isTimeAllowedInclusive(
  ...
  if (!_isMinuteSelectable(time.minute, minuteInterval)) {
    return false;
  }
...
int? _minAllowedMinuteInHourInclusive(
  ...
  for (int minute = 0; minute < 60; minute++) {
    if (!_isMinuteSelectable(minute, minuteInterval)) {
      continue;
    }
    if (_isTimeAllowedInclusive(
...
bool _isAnyTimeAllowedInPeriodInclusive(
  ...
  for (int hour = startHour; hour <= endHour; hour++) {
    if (_minAllowedMinuteInHourInclusive(
          hour,
          minTime,
          maxTime,
          disabledRanges,
...
TimeOfDay _timeOfDayFromMinutes(int minutes) {
  ...
  final int normalized = ((minutes % minutesPerDay) + minutesPerDay) % minutesPerDay;
  return TimeOfDay(hour: normalized ~/ 60, minute: normalized % 60);
}

bool _isMinutesInRangeInclusive(int minutes, int minMinutes, int maxMinutes) {
...
int _clampMinutesToRangeInclusive(int minutes, int minMinutes, int maxMinutes) {
  ...
  if (minMinutes <= maxMinutes) {
    return minutes < minMinutes ? minMinutes : maxMinutes;
  }

  // Range crosses midnight. Invalid times are in (maxMinutes, minMinutes).
  // Clamp to the closest boundary (maxMinutes or minMinutes).
  final int distanceToMax = minutes - maxMinutes; // minutes is > maxMinutes here.
  final int distanceToMin = minMinutes - minutes; // minutes is < minMinutes here.
  return distanceToMax <= distanceToMin ? maxMinutes : minMinutes;
}

TimeOfDay _clampTimeOfDayToRangeInclusive(TimeOfDay time, TimeOfDay minTime, TimeOfDay maxTime) {
  final int minutes = _timeOfDayToMinutes(time);
  final int minMinutes = _timeOfDayToMinutes(minTime);
  final int maxMinutes = _timeOfDayToMinutes(maxTime);
...
class _TimePickerModel extends InheritedModel<_TimePickerAspect> {
  ...
  static MinuteInterval? minuteIntervalOf(BuildContext context) =>
      of(context, _TimePickerAspect.minuteInterval).minuteInterval;
  ...
  bool updateShouldNotifyDependent(
    ...
    if (minuteInterval != oldWidget.minuteInterval &&
        dependencies.contains(_TimePickerAspect.minuteInterval)) {
      return true;
    }
    if (minTime != oldWidget.minTime && dependencies.contains(_TimePickerAspect.minTime)) {
      return true;
    }
...
class _MinuteControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final int minuteStep = _TimePickerModel.minuteIntervalOf(context)?.minutes ?? 1;
    final TimeOfDay nextMinute = selectedTime.replacing(
      minute: (selectedTime.minute + minuteStep) % TimeOfDay.minutesPerHour,
    );
...
class _DayPeriodControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool amEnabled =
        _isAnyTimeAllowedInPeriodInclusive(
          DayPeriod.am,
          model.minTime,
          model.maxTime,
          model.disabledRanges,
...
class _AmPmButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle? resolvedTextStyle = WidgetStateProperty.resolveAs<TextStyle?>(
      ...
      states,
    )?.copyWith(color: enabled ? resolvedTextColor : _withOpacity(resolvedTextColor, 0.38));
    final TextScaler buttonTextScaler = MediaQuery.textScalerOf(context).clamp(maxScaleFactor: 2.0);

    return Material(
      color: resolvedBackgroundColor,
      child: InkWell(
        onTap: enabled ? Feedback.wrapForTap(onPressed!, context) : null,
        child: Semantics(
          checked: selected,
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  TimeOfDay _getTimeForTheta(double theta, {bool roundMinutes = false, required double radius}) {
    ...
    switch (widget.hourMinuteMode) {
      ...
      case _HourMinuteMode.minute:
        ...
        final MinuteInterval? minuteInterval = widget.minuteInterval;
        if (minuteInterval != null) {
          minute = _roundMinuteToInterval(minute, minuteInterval.minutes);
        } else if (roundMinutes) {
          // Round the minutes to nearest 5 minute interval (default dial behavior).
          minute = _roundMinuteToInterval(minute, 5);
        }
  ...
  void _handlePanStart(DragStartDetails details) {
    ...
    final bool snapMinutes = widget.hourMinuteMode == _HourMinuteMode.minute && widget.minuteInterval != null;
    _updateThetaForPan(roundMinutes: snapMinutes);
    _notifyOnChangedIfNeeded(roundMinutes: snapMinutes);
  }
  ...
  void _handleTapUp(TapUpDetails details) {
    ...
    final TimeOfDay tapped = _getTimeForTheta(
      _theta.value,
      roundMinutes: true,
      radius: _radius.value,
    );
    ...
    if (widget.hourMinuteMode == _HourMinuteMode.hour) {
      ...
      switch (widget.hourDialType) {
        case _HourDialType.twentyFourHour:
        case _HourDialType.twentyFourHourDoubleRing:
          _announceToAccessibility(context, localizations.formatDecimal(adjusted.hour));
        case _HourDialType.twelveHour:
          _announceToAccessibility(context, localizations.formatDecimal(adjusted.hourOfPeriod));
      }
      // Always advance to minute selection after a valid hour tap, even if the
      // hour didn't change.
      widget.onHourSelected?.call();
      ...
      if (tapped != widget.selectedTime) {
        widget.onChanged?.call(tapped);
      }
      _announceToAccessibility(context, localizations.formatDecimal(tapped.minute));
      _animateTo(_getThetaForTime(tapped), _getRadiusForTime(tapped));
    }
    _dragging = false;
    _position = null;
    _center = null;
  ...
  Color? _highlightColorForHourAllowedInclusive(int hour, HighlightSpec? highlightSpec) {
    ...
    for (final HighlightedTimeRange range in highlightSpec.ranges) {
      ...
      final int end = _timeOfDayToMinutes(range.end);
      for (int minute = 0; minute < 60; minute++) {
        if (!_isMinuteSelectable(minute, widget.minuteInterval)) {
          continue;
        }
  ...
  List<_TappableLabel> _buildMinutes({
    ...
    final int step = widget.minuteInterval?.minutes ?? 5;
    final List<TimeOfDay> minuteMarkerValues = <TimeOfDay>[
      for (int minute = 0; minute < 60; minute += step) TimeOfDay(hour: 0, minute: minute),
    ];
...
class CustomTimePickerDialog extends StatefulWidget {
  ...
  /// The minimum selectable time (inclusive).
  final TimeOfDay minTime;

  /// The maximum selectable time (inclusive).
  ///
  ...
  /// If set, restricts selection to minutes that are multiples of this value.
  ///
  /// Supported values are: 5, 10, 15, 20, 30.
  final MinuteInterval? minuteInterval;
...
class _CustomTimePickerDialogState extends State<CustomTimePickerDialog> with RestorationMixin {
...
Future<TimeOfDay?> showCustomTimePicker({
  ...
  const TimeOfDay defaultMinTime = TimeOfDay(hour: 0, minute: 0);
...
class _TimePickerDefaultsM2 extends _TimePickerDefaults {
  ...
  BorderSide get dayPeriodBorderSide {
    return BorderSide(
      color: Color.alphaBlend(_withOpacity(_colors.onSurface, 0.38), _colors.surface),
    );
  }
  ...
  Color get dayPeriodColor {
    return WidgetStateColor.resolveWith((Set<WidgetState> states) {
      if (states.contains(WidgetState.selected)) {
        return _withOpacity(_colors.primary, _colors.brightness == Brightness.dark ? 0.24 : 0.12);
      }
  ...
  Color get dayPeriodTextColor {
    return WidgetStateColor.resolveWith((Set<WidgetState> states) {
      return states.contains(WidgetState.selected)
          ? _colors.primary
          : _withOpacity(_colors.onSurface, 0.60);
    });
  }
  ...
  @override
  Color get dialBackgroundColor {
    return _withOpacity(_colors.onSurface, _colors.brightness == Brightness.dark ? 0.12 : 0.08);
  }
  ...
  @override
  Color get entryModeIconColor {
    return _withOpacity(_colors.onSurface, _colors.brightness == Brightness.dark ? 1.0 : 0.6);
  }
  ...
  Color get hourMinuteColor {
    return WidgetStateColor.resolveWith((Set<WidgetState> states) {
      return states.contains(WidgetState.selected)
          ? _withOpacity(_colors.primary, _colors.brightness == Brightness.dark ? 0.24 : 0.12)
          : _withOpacity(_colors.onSurface, 0.12);
    });
  }
  ...
  InputDecorationTheme get inputDecorationTheme {
    return InputDecorationTheme(
      ...
      hintStyle: hourMinuteTextStyle.copyWith(color: _withOpacity(_colors.onSurface, 0.36)),
...
class _TimePickerDefaultsM3 extends _TimePickerDefaults {
  ...
  @override
  TextStyle get helpTextStyle {
    return _textTheme.labelMedium!.copyWith(color: _colors.onSurfaceVariant);
  }
    ...
    return WidgetStateColor.resolveWith((Set<WidgetState> states) {
      if (states.contains(WidgetState.selected)) {
        ...
        if (states.contains(WidgetState.pressed)) {
          ...
          overlayColor = _withOpacity(_colors.onPrimaryContainer, hoverOpacity);
        } else if (states.contains(WidgetState.focused)) {
          const double focusOpacity = 0.1;
          overlayColor = _withOpacity(_colors.onPrimaryContainer, focusOpacity);
        }
        ...
        if (states.contains(WidgetState.pressed)) {
          ...
          const double hoverOpacity = 0.08;
          overlayColor = _withOpacity(_colors.onSurface, hoverOpacity);
        } else if (states.contains(WidgetState.focused)) {
          const double focusOpacity = 0.1;
          overlayColor = _withOpacity(_colors.onSurface, focusOpacity);
        }
